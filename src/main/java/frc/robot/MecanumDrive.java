...
/** A class used to interface with a mecanum wheel drivetrain. */
public class MecanumDrive {
  ...
  /**
   * Adjust rotation velocity and directional velocity. Expected input range: -1 to 1.
   *
   * @param forward forward velocity
   * @param strafe rightward velocity
   * @param turn clockwise rotation velocity
   */
  public void speed(double forward, double strafe, double turn) {
    // This ensures the absolute value sum of all
    // velocities is 1 or under.
    double forwardR = forward / (1 + Math.abs(strafe) + Math.abs(turn));
    double strafeR = strafe / (1 + Math.abs(forward) + Math.abs(turn));
    double turnR = turn / (1 + Math.abs(strafe) + Math.abs(forward));
